...
if _Spec == 2 and CML_Cooldowns_Config == nil then
	...
	CML_Cooldowns_Config = {
		...
		abilities = {
			{ 	name	= "Auto Taunts",
				tooltip	= PlayerHex.."Toggle |cffFFFFFFAuto Taunts for"..PlayerHex.."Siege of Ogrimar Fights. |cffFFFFFFOriginal code by Tao and Rubim.",
				enable	= true,
			},	
			{ 	name	= "Lights Hammer",
				tooltip	= PlayerHex.."Toggle |cffFFFFFFLight's Hammer "..PlayerHex.."on cursor.",
...
if not PaladinFunctions then
	...
	function CML.EternalFlame()
		if _G[PQIprefix.."WordOfGlory_enable"] then
			for i = 1, #nNova do
